...
FalconCollectionsContentsWrapper::FalconCollectionsContentsWrapper(Profile* profile)
    : WebUIContentsWrapper(GURL(base::StrCat({"chrome://",
                           ...
                           std::string(FalconCollectionsPanelUI::GetWebUIName())),
      webui_url_(
          base::StrCat({"chrome://", falcon::kFalconCollectionsPanelHost})) {
  CHECK(GetWebUIController());
  GetWebUIController()->set_embedder(weak_ptr_factory_.GetWeakPtr());
}

FalconCollectionsContentsWrapper::~FalconCollectionsContentsWrapper() = default;

void FalconCollectionsContentsWrapper::ReloadWebContents() {
...
FalconCollectionsContentsWrapper::GetWeakPtr() {
  return weak_ptr_factory_.GetWeakPtr();
}

FalconCollectionsPanelUI* FalconCollectionsContentsWrapper::GetWebUIController() {
  content::WebUI* const webui = web_contents()->GetWebUI();
  return webui && webui->GetController()
             ? webui->GetController()->GetAs<FalconCollectionsPanelUI>()
             : nullptr;
}

BEGIN_METADATA(FalconCollectionsSidePanelWebView)
END_METADATA

// static
std::unique_ptr<views::View> FalconCollectionsSidePanelWebView::CreateView(
    Profile* profile,
    SidePanelEntryScope& scope) {
  CHECK(profile);
  auto contents_wrapper =
      std::make_unique<FalconCollectionsContentsWrapper>(profile);
  auto web_view = std::make_unique<FalconCollectionsSidePanelWebView>(
      scope, std::move(contents_wrapper));
  web_view->ShowUI();
  return web_view;
}

FalconCollectionsSidePanelWebView::FalconCollectionsSidePanelWebView(
    SidePanelEntryScope& scope,
...
FalconCollectionsSidePanelWebView::~FalconCollectionsSidePanelWebView() = default;
